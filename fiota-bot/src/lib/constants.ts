/**
 * CONSTANTS - Chapters, Industries, and other standardized values
 *
 * Source: phiota.org/chapters (chapters), NAICS simplified (industries)
...
export interface Chapter {
    ...
    hidden: boolean;         // If true, excluded from verification autocomplete
}
...
/**
 * Get chapters for verification autocomplete (excludes hidden chapters like Omega)
 */
...
/**
 * Get all chapters including hidden ones (for E-Board commands)
 */
...
/**
 * Find chapter by value (kebab-case ID)
 */
export function getChapterByValue(value: string): Chapter | undefined {
    return CHAPTERS.find(ch => ch.value.toLowerCase() === value.toLowerCase());
}

/**
 * Search chapters by query (for autocomplete)
 * Matches against label, institution, and state
 */
export function searchChapters(query: string, includeHidden = false): Chapter[] {
    const q = query.toLowerCase();
    return CHAPTERS.filter(ch => {
        if (ch.hidden && !includeHidden) return false;
        return (
            ch.label.toLowerCase().includes(q) ||
            ch.institution.toLowerCase().includes(q) ||
...
/**
 * Search industries by query (for autocomplete)
 */
export function searchIndustries(query: string): string[] {
    const q = query.toLowerCase();
    return INDUSTRIES.filter(ind => ind.toLowerCase().includes(q)).slice(0, 25);
}

/**
 * Validate if a chapter value exists
 */
export function isValidChapter(value: string): boolean {
    return CHAPTERS.some(ch => ch.value.toLowerCase() === value.toLowerCase());
}

/**
 * Validate if an industry exists in the list
 */
